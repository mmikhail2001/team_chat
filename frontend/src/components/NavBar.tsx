...
export default function NavBar() {
    ...
    return (
        <div className={`my-3 h-12 w-full md:h-full md:w-12 ${ hideNavBar ? 'hidden' : 'flex' } md:flex md:flex-col items-center justify-evenly md:justify-start border-t md:border-t-0 md:border-r border-zinc-800`}>
    <img src={user.avatar} alt="avatar" className="w-8 h-8 m-2 bg-zinc-300 hover:cursor-pointer rounded-xl" style={{ objectFit: 'cover', objectPosition: 'center' }} onError={setDefaultAvatar} onClick={ () => navigate("/") } />
    ...
    { !user.is_guest && <IoMailOpenOutline size={32} className={`m-2 hover:opacity-90 ${isActiveIcon('/mailings') ? 'opacity-80' : 'opacity-30'}`} onClick={ () => navigate("/mailings") } />}
