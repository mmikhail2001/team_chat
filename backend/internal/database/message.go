...
func (db *Database) CreateMessage(content string, channel_id string, system_message bool, user *User) (*Message, int) {
	...
	if statusCode != http.StatusOK {
		log.Println("CreateMessage: statusCode: ", statusCode)
		return nil, statusCode
	}

	if channel.Type == 1 {
		for _, recipient := range channel.Recipients {
			// какие-то мучения постоянно с этим непонятным порядком user_id и recipient_id в массиве участников личного чата
	...
	if err != nil {
		log.Println("CreateMessage: InsertOne: err: ", err)
...
func (db *Database) EditMessage(id string, channel_id string, content string, user *User) (*Message, int) {
	...
	if statusCode != http.StatusOK {
		log.Println("EditMessage: GetMessage: statusCode: ", statusCode)
		return nil, statusCode
	}
	...
	if err != nil {
		log.Println("EditMessage: UpdateOne: err: ", err)
		return nil, http.StatusInternalServerError
	}
...
func (db *Database) GetMessage(id string, channel_id string, user *User) (*Message, *Channel, int) {
	...
	if err != nil {
		log.Println("GetMessage: FindOne: err: ", err)
...
func (db *Database) GetMessages(channel_id string, limit int64, offset int64, user *User) ([]Message, int) {
	...
	if err != nil {
		log.Println("GetMessages: Find: err: ", err)
		return nil, http.StatusInternalServerError
	}
