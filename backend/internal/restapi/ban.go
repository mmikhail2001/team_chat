...
func GetAllBans(ctx *Context) {
	log.Println("handler GetAllBans")
	...
	for cur.Next(context.TODO()) {
		...
		BannedBy, statusCode := ctx.Db.GetUser(ban.BannedBy)
		if statusCode != http.StatusOK {
			continue
		}

		BannedUser, statusCode := ctx.Db.GetUser(ban.BannedUser)
		if statusCode != http.StatusOK {
			continue
		}
