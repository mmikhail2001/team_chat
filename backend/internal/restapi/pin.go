...
func GetPins(ctx *Context) {
	...
	for _, message := range pinned_messages {
		...
		author, statstatusCode := ctx.Db.GetUser(message.AccountID)
...
func PinMsg(ctx *Context) {
	...
	author, statusCode := ctx.Db.GetUser(message.AccountID)
	if statusCode != http.StatusOK {
		return
	}
