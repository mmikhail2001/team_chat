...
func GetRelationships(ctx *Context) {
	...
	for _, relationship := range relationships {
		...
		if relationship.Type == 1 {
			status = ctx.Conn.GetUserStatus(user.ID)
		}
...
func GetRelationship(ctx *Context) {
	log.Println("handler GetRelationship")
...
func ChangeRelationshipToDefault(ctx *Context) {
	log.Println("handler ChangeRelationshipToDefault")
	url_vars := mux.Vars(ctx.Req)
	...
	res := response.NewRelationship(response.NewUser(relationship_user, ctx.Conn.GetUserStatus(relationship_user.ID)), 0)
	ctx.WriteJSON(res)
	status := 0
	if relationship_user_type == 1 {
		status = ctx.Conn.GetUserStatus(ctx.User.ID)
	}
	ctx.Conn.SendToUser(relationship_user.ID, "RELATIONSHIP_MODIFY", response.NewRelationship(response.NewUser(&ctx.User, status), relationship_user_type))
}

func ChangeRelationshipToFriend(ctx *Context) {
	log.Println("handler ChangeRelationshipToFriend")
	url_vars := mux.Vars(ctx.Req)
	relationship_id := url_vars["rid"]
	relationshipsCollection := ctx.Db.Mongo.Collection("relationships")
	...
	res := response.NewRelationship(response.NewUser(relationship_user, ctx.Conn.GetUserStatus(relationship_user.ID)), relationship_type)
	ctx.WriteJSON(res)
	status := 0
	if relationship_user_type == 1 {
		status = ctx.Conn.GetUserStatus(ctx.User.ID)
...
func ChangeRelationshipToBlock(ctx *Context) {
	log.Println("handler ChangeRelationshipToBlock")
	url_vars := mux.Vars(ctx.Req)
	relationship_id := url_vars["rid"]
	relationshipsCollection := ctx.Db.Mongo.Collection("relationships")
	...
	res := response.NewRelationship(response.NewUser(relationship_user, ctx.Conn.GetUserStatus(relationship_user.ID)), 2)
	ctx.WriteJSON(res)
	status := 0
	if relationship_user_type == 1 {
		status = ctx.Conn.GetUserStatus(ctx.User.ID)
